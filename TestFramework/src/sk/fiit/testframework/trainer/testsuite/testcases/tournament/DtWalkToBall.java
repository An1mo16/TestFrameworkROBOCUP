...
public class DtWalkToBall implements Runnable, ITestCaseObserver {
	...
	//	lowSkills.put("step_circ_right", 53);
		...
		for(int i = 1; i <= populationLength; i++){
			Population.add("dtWalk2Ball" + i);
			 List<Double> data = XmlToData(Population.get(i-1));
		...
		for(int k = 0; k < maxGenerations; k++){
			...
			for(int i = 0; i < populationLength; i++){
				 List<Double> data = XmlToData(Population.get(i));
				PopulationData.set(i, data);
			}
			...
			for(int i = 0; i < populationLength; i++){
				...
				impl.enqueueTestCase(new DtWalkToBallTest(Population.get(i)), this);	 // tu sa vytvori parameter kde sa vlozi nazov xml suboru kde je rozhodovaci strom
			}
			while(testResults.size() != populationLength){
				try {
					Thread.sleep(1000);
			...
			for(int i = 0; i < populationLength; i++){
				if (maxFittnes < testResults.get(i).getFitness()){
					maxFittnes = testResults.get(i).getFitness();
					maxFittnesFile = Population.get(i);
				}
			}
